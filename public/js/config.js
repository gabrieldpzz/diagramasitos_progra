function getAttributeForm(DB, active_table) {
	...
	attribute_form += "<div class='form-group'><label for='type-dropdown'>Tipo de dato:</label><br>";

	// from config.js
	attribute_form += attributeList;
	attribute_form += '</div>';

	attribute_form += "<label for='size-input'>Tamaño:</label><br><input type='text' id='size-input' class='form-control size' />";
	attribute_form += "<label>Restrincción:</label><div class='attribute_constraints'>";
